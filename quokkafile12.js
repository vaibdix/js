...
// Using map
let a = 0
let addition = arr.map(val => { return a+=val  })
...
let val = arr.find((val, i, wa) => { return val > 20 })
console.log(val);
...
// use data from api
let val1 = data.find((val) => { return val.first_name == "Jimmie" })
console.log(val1);
...
let cartItems = []
let val2 = data.find((val) => { return val.id == "10" })
console.log(val2);
cartItems.push(val2)
...
data.filter(val => {return( val.gender == "Male")})
